                    ...
                    @foreach($cart as $id => $details)
                        <tr>
                            <td style="width: 100px;">
                                @if($details['image'])
                                    <img src="{{ asset('storage/' . $details['image']) }}" width="80" style="object-fit: contain;">
                                @else
                            ...
                            <td style="vertical-align: middle; width: 120px;">
                                <input type="number" value="{{ $details['quantity'] }}" class="form-control text-center" min="1" disabled>
                            </td>
                            ...
                            <td style="vertical-align: middle;">
                                <form action="{{ route('cart.remove', $id) }}" method="POST" onsubmit="return confirm('Silmək istədiyinizdən əminsiniz?')">
                                    @csrf
                                    @method('DELETE')
            ...
            <div class="row" style="margin-top: 20px;">
                <div class="col-sm-4 col-sm-offset-8" style="text-align: right;">
                    <div class="well" style="background: #f5f5f5; padding: 20px; border-radius: 6px;">
                        ...
                        <hr>
                        <a href="{{ $whatsAppLink }}" target="_blank" class="btn btn-primary btn-lg btn-block" style="background: #333; border: none;">Sifarişi Tamamla</a>
                    </div>
                </div>
            </div>
        @else
            <div class="text-center" style="padding: 50px 0;">
                ...
                <a href="{{ route('web.product') }}" class="btn btn-warning" style="background: #ffbc00; color: #000; border: none; font-weight: 600;">   {{ t('continue_shopping') }}</a>
            </div>
        @endif
    </div>
</div>
